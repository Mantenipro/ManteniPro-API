...
router.get('/user', authMiddleware, async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, error: 'Error retrieving user equipment' })
  }
})
...
router.put('/:id', authMiddleware, async (req, res) => {
  try {
    ...
    const updatedEquipment = await equipmentUseCase.editEquipment(
      equipmentId,
      updatedData
    )
    ...
    return res
      .status(500)
      .json({ success: false, error: 'Error editing equipment' })
  }
})
...
router.get('/', authMiddleware, async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, error: 'Error retrieving equipment list' })
  }
})
...
router.get('/company/:companyId', authMiddleware, async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, error: 'Error retrieving equipment by company' })
  }
})
...
router.get('/:id', authMiddleware, async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, error: 'Error retrieving equipment by ID' })
  }
})
